        ...
        rx.menu.root(
            rx.menu.trigger(
                rx.button(
                    ...
                    variant="soft",
                    on_click=lambda: rx.console_log("State button clicked"),  # Debug click
                ),
            ),
            rx.menu.content(
                rx.foreach(
                    State.states_keys,
                    lambda state, _: rx.menu.item(
                        state,
                        on_click=lambda: [
                            State.set_state(state),
                            rx.console_log(f"State selected: {state}")  # Debug state selection
                        ]
                    )
                )
            ),
        ...
        rx.menu.root(
            rx.menu.trigger(
                rx.button(
                    ...
                    variant="soft",
                    is_disabled=(State.state_value == ""),
                    on_click=lambda: rx.console_log("Address button clicked"),  # Debug click
                ),
            ),
            rx.menu.content(    
                rx.cond(
                    ...
                    rx.foreach(
                        State.addresses,
                        lambda address, _: rx.menu.item(
                            address,
                            on_click=lambda: [
                                State.set_address(address),
                                rx.console_log(f"Address selected: {address}")  # Debug address selection
                            ]
                        )
                    ),
                    rx.menu.item("Please select a state first"),
                )
            ),
        ),
        rx.button(
            ...
            is_disabled=rx.cond(
                (State.state_value == "") | (State.address_value == ""),
                True,
                False,
            )
        ),
